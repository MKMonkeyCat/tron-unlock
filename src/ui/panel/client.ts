...
const getLeafId = (id: FeatureId): string => id.split('.').pop() ?? id;
...
export const resolveFeatureLabel = (
  ...
  const dict = currentLocaleDict(
    groupI18n.translations as Record<
      string,
...
export const resolveCategoryLabel = (
  ...
  const dict = currentLocaleDict(
    categoryI18n.translations as Record<string, { name?: string }>,
  );
...
export const resolveGroupLabel = (
  ...
  const categoryI18n = registry.i18n.getCategoryI18nContext(category);
  const dict = currentLocaleDict(
    categoryI18n.translations as Record<
      string,
